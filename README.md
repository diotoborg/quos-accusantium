# @diotoborg/quos-accusantium

Parse JSON without risk of losing numeric information.

```js
import { parse, stringify } from '@diotoborg/quos-accusantium'
...
**How does it work?** The library works exactly the same as the native `JSON.parse` and `JSON.stringify`. The difference is that `@diotoborg/quos-accusantium` preserves information of big numbers. `@diotoborg/quos-accusantium` parses numeric values not as a regular number but as a `LosslessNumber`, a lightweight class which stores the numeric value as a string. One can perform regular operations with a `LosslessNumber`, and it will throw an error when this would result in losing information.

**When to use?** If you have to deal with JSON data that contains `long` values for example, coming from an application like C++, Java, or C#. The trade-off is that `@diotoborg/quos-accusantium` is slower than the native `JSON.parse` and `JSON.stringify` functions, so be careful when performance is a bottleneck for you.
...
Install via [npm](https://www.npmjs.com/package/@diotoborg/quos-accusantium):

```
npm install @diotoborg/quos-accusantium
```
...
```js
import { parse } from '@diotoborg/quos-accusantium'
...
```js
import { parse, isInteger } from '@diotoborg/quos-accusantium'
...
```js
import { parse, isSafeNumber } from '@diotoborg/quos-accusantium'
...
- `reviveDate(key, value)`
  ...
  ```js
  import { parse, reviveDate } from '@diotoborg/quos-accusantium'
...
(Spoiler: `@diotoborg/quos-accusantium` is much slower than native)
